...
public final class GaugeOilTemperature extends View {

	private static final String TAG = GaugeOilTemperature.class.getSimpleName();
	...
	private static final String mTitle = "Oil";
	...
	private static final int scaleMaxValue = 130;
	...
	public GaugeOilTemperature(Context context) {
		super(context);	    
		init();
	}

	public GaugeOilTemperature(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public GaugeOilTemperature(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initDrawingTools() {
		...
		// We need to create a path to define the moving needle. Let's keep it
		// pretty simply to begin with
		handPath = new Path();
		handPath.moveTo(0.5f, 0.0f);
		handPath.lineTo(0.5f, 0.5f + 0.4f);
		handPath.addCircle(0.5f, 0.0f, 0.025f, Path.Direction.CW);
	}
	...
	private Runnable mUpdateTimeTask = new Runnable() {
		public synchronized void run() {
			int oiltemp = ((DashApplication) myApp).getOilTemp();
			
			if ((oiltemp >= scaleMinValue) && (oiltemp <= scaleMaxValue))
				setHandTarget(oiltemp);
